      ...
      <div className="flex w-full max-w-3xl flex-col items-center px-6 text-center">
        <div className="day-splash-chapter flex items-center gap-3">
          ...
          <span className="text-left">
            <span className="block text-sm font-bold text-[#f6f0f7]/80">IEEE RAS HTU Student Chapter</span>
            <span className="block text-sm text-[#f6f0f7]/60">presents</span>
          </span>
        </div>

        <div className="day-splash-lights mt-12 rounded-[6px] border border-white/10 bg-black/40 p-4">
          <div className="flex gap-3 sm:gap-4">
            {[0, 1, 2, 3, 4].map((i) => (
              <div key={i} className="flex flex-col gap-2 rounded-[4px] bg-[#1a0b1f] p-2">
        ...
        <div className="relative mt-12 w-full">
          <p className="day-splash-name font-brand text-6xl leading-none text-[#f6f0f7] sm:text-8xl">MMRC 26</p>
          <p className="day-splash-name day-display mt-4 text-2xl text-[#ff8aa2] sm:text-3xl">Competition day</p>
          <div className="relative mx-auto mt-8 h-1.5 w-full max-w-md overflow-hidden">
            <div className="day-splash-streak absolute inset-0 bg-[#ff8aa2]" />
          </div>
          <div className="pointer-events-none absolute inset-x-0 bottom-[-6px] h-6 overflow-hidden">
            <MouseMark className="day-splash-mouse absolute h-6 w-6 text-[#f6f0f7]" />
          </div>
        </div>

        <p className="mt-14 text-xs text-[#f6f0f7]/60">Tap to skip</p>
      </div>

      <div className="day-splash-flag day-checker" style={{ ["--size" as string]: "22px", color: "#f6f0f7", ["--day-ink" as string]: "246 240 247" }} />
    </div>
  );
}
