        ...
        <section className="space-y-6">
          ...
          <ul className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4">
            {members.map((member, index) => (
              <li key={member.id} className="day-card day-posts flex flex-col items-center p-5 text-center" data-reveal style={{ ["--i" as string]: index % 8 }}>
                {member.photoUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element -- small committee photos from the blob store, already sized on upload
                  <img src={member.photoUrl} alt="" className="h-20 w-20 rounded-[3px] object-cover" />
                ) : (
                  <span className="day-display grid h-20 w-20 place-items-center rounded-[3px] bg-day-plum/10 text-2xl text-day-plum">
